      ...
      <div className="clientProfileButton">                   
        <p className="bold">{c.Name}</p>
          <table className="profileInfo">
            <tbody>
              {buttonStates[0] && <tr><td className="bold profileInfo">ID:</td> <td>{c.ID}</td></tr>}
              {buttonStates[1] && <tr><td className="bold profileInfo">Email:</td> <td>{c.Email}</td></tr>}
              {buttonStates[2] && <tr><td className="bold profileInfo">Foster Certification:</td><td>#{c.FosterCert}</td></tr> }
              {buttonStates[3] && <tr><td className="bold profileInfo">Adoption Certification:</td> <td>#{c.AdoptCert}</td></tr>}
            </tbody>
          </table>
        </div>
      )
    ...
    async function GetProjection() {
      try {
        ...
        const newObjects = data.data.map(row => ({
          ...
          FosterCert : buttonStates[2] ? row[getChosenOptionIndex(chosenOptions, "FosterPersonCertificationID")] : null,
          AdoptCert : buttonStates[3] ? row[getChosenOptionIndex(chosenOptions, "AdopterPersonCertificationID")] : null,
        }));
